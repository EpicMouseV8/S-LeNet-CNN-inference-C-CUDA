...
public:
...

	{
		...
		//converting from [6][25] to [6][5][5]
		for (int filter = 0; filter < 6; ++filter) {
			for (int i = 0; i < 25; ++i) {
				int row = i / 5; // Determine the row in the 5x5 matrix
				int col = i % 5; // Determine the column in the 5x5 matrix
				weights[filter][row][col] = c1_weight[filter][i];
			}
		}

		float bias[6] = {-0.295779, 0.430410, 0.389516, -0.437929, 0.249217, -0.041092,};
		...
		// SS layer weights and biases
		cudaMalloc((void**)&d_pre_output_ss1, 6 * 6 * 6 * sizeof(float));
		cudaMalloc((void**)&d_weights_ss1, 1 * 4 * 4 * sizeof(float));

		//from [1][16] to [1][4][4]
		float s2_weight[1][16] = { {-0.806280, -3.872459, -2.141078, -6.543421, 
			-2.135209, -5.067534, -2.430283, -6.847790, 
			2.860572, -2.327538, 0.649373, -1.243741,
			7.845700, 5.709686, 5.967687, 5.219587, },
		};

		float weights_ss1[1][4][4];

		for (int i = 0; i < 16; ++i) {
			int row = i / 4;
			int col = i % 4; 
			weights_ss1[0][row][col] = s2_weight[0][i];
		}


		cudaMemcpy(d_weights_ss1, weights_ss1, 1 * 4 * 4 * sizeof(float), cudaMemcpyHostToDevice);

		float bias_ss1[1] = { 0.827946 };
		...
		float weights_fc[10][6][6][6];

		//from [10][216] to [10][6][6][6]
		for (int neuron = 0; neuron < 10; ++neuron) {
			for (int i = 0; i < 216; ++i) {
				...
				weights_fc[neuron][c][h][w] = f3_weight[neuron][i];
			}
		};
		...
		float bias_fc[10] = { -4.136192, 1.261294, -3.176764, -4.166682, -4.117179, -1.692405, -3.250649, -2.580800, -11.371058, -6.880490, };
	...
	void forward_pass (float data[28][28]) {

		/*CONVOLUTION*/

		float (*input)[28];
		cudaMalloc((void**)&input, 28 * 28 * sizeof(float));
		cudaMemcpy(input, data, 28 * 28 * sizeof(float), cudaMemcpyHostToDevice);

		dim3 threadsPerBlock(12, 12); // A block of 24x24 threads
		dim3 numBlocks(2, 2, 6); // 6 blocks in the z-dimension

		// Assuming input, pre_output, and weight are already allocated and initialized in device memory
		...
		dim3 threadsPerBlockSS(2, 2); // Can be changed based on your specific requirements
		dim3 numBlocksSS(3, 3, 6); // Covering the 6x6x6 output space
		...
		// Assuming pre_output and bias are already allocated and populated on the device
		...
		/*FC MAPPING to dim 10*/
		dim3 threadsPerBlockFC(10);
		dim3 numBlocksFC(1);
		...
		cudaDeviceSynchronize();
		float output_fc[10];
		cudaMemcpy(output_fc, d_output_fc, 10 * sizeof(float), cudaMemcpyDeviceToHost);
...
static void read_mnist_imageset(const char* image_filename, struct mnist_data** data_set, unsigned int* count) {
	...
	imageset = fopen(image_filename, "rb");
	...
	int magic_number;
	fread(&magic_number, sizeof(int), 1, imageset);
	magic_number = reverseBytes(magic_number); // Swap endianness

	int num_images;
	fread(&num_images, sizeof(int), 1, imageset);
	num_images = reverseBytes(num_images);

	int num_rows;
	fread(&num_rows, sizeof(int), 1, imageset);
	num_rows = reverseBytes(num_rows);

	int num_cols;
	fread(&num_cols, sizeof(int), 1, imageset);
	...
	for (int i = 0; i < num_images; i++) {
		...
		//store each bin as double
		int temp = 0;
		for (int j = 0; j < 28; j++) {
			for (int k = 0; k < 28; k++) {
				data_set[i]->data[j][k] = (double)image_data[temp] / 255.0;
				temp++;
			}
		}
	}
	...
	printf("The Magic number of images: %d; \n", magic_number);
...
static void read_mnist_labels(const char* label_filename, struct mnist_data** data_set, unsigned int* count) {
	...
	fread(&magic_number_labels, sizeof(int), 1, labels);
	magic_number_labels = reverseBytes(magic_number_labels); // Swap endianness
	...
	for (int i = 0; i < num_labels; i++) {
		fread(&label, sizeof(unsigned char), 1, labels);
		(data_set[i])->label = label;
	}
...
static int mnist_load(const char* image_filename, const char* label_filename, struct mnist_data** data_set, unsigned int* count) {
...
void convertDoubleArrayToFloatArray(const double input[28][28], float output[28][28]) {
	for (int i = 0; i < 28; ++i) {
		for (int j = 0; j < 28; ++j) {
